...
function convertToPropTypes(node, importedTypes, internalTypes) {
  ...
  if (node.type === 'ObjectTypeAnnotation') {
    ...
    var value = node.value; // recurse

    value = convertToPropTypes(value, importedTypes, internalTypes); // handles id?: string
    ...
    var _properties = spreadShape.properties; // Unless or until the strange default behavior changes in flow (https://github.com/facebook/flow/issues/3214)
    ...
    return _properties;
  } else if (node.type === 'FunctionTypeAnnotation') resultPropType = {
    type: 'func'
  };else if (node.type === 'AnyTypeAnnotation') resultPropType = {
    type: 'any'
  }; // babylon6 Node Name
  else if (node.type === 'ExistentialTypeParam') resultPropType = {
      type: 'any'
    }; // babylon7 Node Name
    else if (node.type === 'ExistsTypeAnnotation') resultPropType = {
        type: 'any'
      };else if (node.type === 'MixedTypeAnnotation') resultPropType = {
        type: 'any'
      };else if (node.type === 'TypeofTypeAnnotation') resultPropType = {
        type: 'any'
      };else if (node.type === 'EmptyTypeAnnotation') resultPropType = {
        type: 'any'
      };else if (node.type === 'NumberTypeAnnotation') resultPropType = {
        type: 'number'
      };else if (node.type === 'StringTypeAnnotation') resultPropType = {
        type: 'string'
      };else if (node.type === 'BooleanTypeAnnotation') resultPropType = {
        type: 'bool'
      };else if (node.type === 'VoidTypeAnnotation') resultPropType = {
        type: 'void'
      };else if (node.type === 'TupleTypeAnnotation') resultPropType = {
        type: 'arrayOf',
        of: {
          type: 'any'
        }
        ...
        if (mergedProperties.length === 0 && requiresRuntimeMerge.length === 0) {
          ...
          if (tp[0].type === 'StringLiteralTypeAnnotation') {
            ...
            if (node.type === 'ArrayTypeAnnotation' || node.id.name === 'Array' || node.id.name === '$ReadOnlyArray') {
              ...
              resultPropType = {
                type: 'raw',
                value: importedTypes[node.id.name].accessNode
              };
            ...
            var optional = typesWithoutNulls.length !== types.length; // e.g. null | string
            //     'foo' | null
            ...
            var _value = node.value; // babylon7 does not provide value for NullLiteralTypeAnnotation
  ...
  if (resultPropType) {
    ...
    throw new Error("".concat(_util.PLUGIN_NAME, ": Encountered an unknown node in the type definition. Node: ").concat(JSON.stringify(node)));
  }
}