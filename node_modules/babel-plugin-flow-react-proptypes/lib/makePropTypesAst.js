...
exports.setMakePropTypeImportNode = setMakePropTypeImportNode;
...
var anyTemplate = (0, _template.default)("\n(props, propName, componentName) => {\n  if (!Object.prototype.hasOwnProperty.call(props, propName)) {\n    throw new Error(`Prop \\`${propName}\\` has type 'any' or 'mixed', but was not provided to \\`${componentName}\\`. Pass undefined or any other value.`);\n  }\n}\n");
var deferTpl = (0, _template.default)("(function() { return (NODE).apply(this, arguments); })");
/**
...
function makeObjectAstForRaw(propTypeSpec, propTypeObjects) {
  var propTypeObject = typeof propTypeSpec.value === 'string' ? t.identifier(propTypeSpec.value) : propTypeSpec.value; // This will just be a variable, referencing an import we
...
function makePropType(data, isExact) {
  ...
  if (method === 'string' || method === 'number' || method === 'bool' || method === 'object' || method === 'array' || method === 'func' || method === 'node') {
    ...
    markFullExpressionAsRequired = false; // In 'raw', we handle variables - typically derived from imported types.
    ...
    var variableNode = typeof data.value === 'string' ? t.identifier(data.value) : t.cloneDeep(data.value);
    var originalVariableNode = variableNode;
    var shapeNode = t.callExpression(t.memberExpression(makePropTypeImportNode(), t.identifier('shape')), [t.cloneDeep(variableNode)]);
    ...
    if (data.isRequired) {
      variableNode = t.conditionalExpression(t.memberExpression(t.cloneDeep(variableNode), t.identifier('isRequired')), t.memberExpression(t.cloneDeep(variableNode), t.identifier('isRequired')), t.cloneDeep(variableNode));
    }
    ...
    node = t.callExpression(t.memberExpression(node, t.identifier('objectOf')), [makePropType(data.of)]);
    ...
    node = t.conditionalExpression(_functionCheckNode, instanceOfNode, anyNode); // Don't add .required on the full expression; we already handled this ourselves
    ...
    throw new Error("".concat(_util.PLUGIN_NAME, " processing error: This is an internal error that should never happen. ") + "Report it immediately with the source file and babel config. Data: ".concat(bugData));
  }
